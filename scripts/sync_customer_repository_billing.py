...
API_VERSION = "2025-04"
BILLING_COLUMNS = ["status", "date_mm4xwgdq", "board_relation_mm4xbyad", "text_mm78aet7", "text_mm78xmh9", "text_mm78rn5r", "date_mm79dy8", "numeric_mm78f2z0", "numeric_mm78eycx", "numeric_mm78mwxt", "color_mm79a62r", "color_mm79payw"]
QUERY = """
query($board_id: ID!, $cursor: String) {
  boards(ids: [$board_id]) {
    ...
    items_page(limit: 500, cursor: $cursor) { cursor items {
      id name
      subitems { id name updated_at column_values(ids: ["status","date_mm4xwgdq","board_relation_mm4xbyad","text_mm78aet7","text_mm78xmh9","text_mm78rn5r","date_mm79dy8","numeric_mm78f2z0","numeric_mm78eycx","numeric_mm78mwxt","color_mm79a62r","color_mm79payw"]) { id text value type ... on BoardRelationValue { linked_items { id name board { id name } } } } }
    }
  }
}
"""
...
def normalize(board,parents):
    ...
    for parent in parents:
        for item in parent.get("subitems") or []:
            ...
            row={"source_board_id":SUBITEM_BOARD,"source_item_id":str(item["id"]),"customer_parent_id":str(parent["id"]),"customer_name":parent["name"],"name":item["name"],"source_updated_at":item.get("updated_at"),"installation_status":c.get("status",{}).get("text") or None,"activation_date":c.get("date_mm4xwgdq",{}).get("text") or None,"inventory_relations":inv,"sap_number":c.get("text_mm78aet7",{}).get("text") or None,"po_number":c.get("text_mm78xmh9",{}).get("text") or None,"invoice_number":c.get("text_mm78rn5r",{}).get("text") or None,"invoice_date":c.get("date_mm79dy8",{}).get("text") or None,"billing_status":c.get("color_mm79payw",{}).get("text") or None,"latest_bill_month":c.get("color_mm79a62r",{}).get("text") or None,"basic_billing_value":c.get("numeric_mm78f2z0",{}).get("text") or None,"gst_rate":c.get("numeric_mm78eycx",{}).get("text") or None,"gross_billing_value":c.get("numeric_mm78mwxt",{}).get("text") or None}
            ...
            if row["billing_status"]=="Billed" and not row["invoice_number"]: row["validation_issues"].append("billed_without_invoice_number")
            if row["billing_status"] in {"Ready to bill","Billed"} and not row["basic_billing_value"]: row["validation_issues"].append("billing_state_without_basic_value")
